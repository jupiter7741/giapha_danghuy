...
async function loadTree() {
    try {
        ...
        // 4. KIỂM SOÁT CAMERA SAU KHI VẼ
        if (Object.keys(expandedStates).length === 0) {
            // Chờ thư viện vẽ DOM xong (250ms là đủ an toàn)
            setTimeout(() => {
                const state = chart.getChartState();
                ...
                // Quét tọa độ thực tế của tất cả các nút đang hiển thị
                // Trong thư viện này, n.x là "Tâm ngang" của nút, n.y là "Đỉnh" của nút
                ...
                // Trọng tâm ngang của toàn bộ cây
                const treeCenterX = minX + (treeWidth / 2);
                
                // Tính tỷ lệ Zoom (Scale) để vừa khít 100% màn hình
                const padding = 30; // Khoảng cách an toàn 2 bên lề
                const scaleX = (state.svgWidth - padding * 2) / treeWidth;
                const scaleY = (state.svgHeight - padding * 2) / treeHeight;
                const scale = Math.min(scaleX, scaleY, 1); // Lấy tỷ lệ nhỏ nhất, tối đa là 1 (không phóng quá to)
                
                // Bước 3: Căn tọa độ X vào chính giữa, tọa độ Y cách mép trên 50px
                const translateX = (state.svgWidth / 2) - (treeCenterX * scale);
                const translateY = 50 - (minY * scale);
                
                // Bước 4: Di chuyển Camera đúng 1 lần duy nhất (Không gây xung đột giật lag)
            ...
            // Căn giữa và Fit lại cây sau khi vẽ xong
            chart.fit();
        }
...
async function saveMember(e) {
    ...
    // Thêm dòng này lấy ID ẩn:
    const memberId = document.getElementById('memberId')?.value;

    // Dùng ?.value để chống lỗi null
    const data = {
    ...
    // NẾU ĐANG SỬA, THÊM ID VÀO PAYLOAD ĐỂ BACKEND UPDATE
    if (memberId) {
        data.id = memberId;
    }
    ...
    if(res.ok) {
        // --- BẮT ĐẦU: TỰ ĐỘNG ĐỒNG BỘ TÊN MẸ CHO CÁC CON ---
        // (Đã khai báo biến memberId ở đầu hàm)
        const newSpouse = document.getElementById('spouse')?.value || "";
        const oldSpouse = document.getElementById('oldSpouse')?.value || "";
        
        // Nếu người dùng có sửa lại tên vợ/chồng thành một tên khác
        if (memberId && newSpouse !== oldSpouse) {
            
            // Tìm tất cả các con của người này đang mang tên Mẹ cũ (hoặc chưa có tên mẹ)
            const childrenToUpdate = familyData.filter(d => 
                d.parent_id === memberId && 
                (d.mother_name === oldSpouse || !d.mother_name)
            );
            
            // Gọi API cập nhật âm thầm từng đứa con bằng tên Vợ mới
        ...
        if (window.isContinue) {
            // Update ngầm cây gia phả ở background
            loadTree();
            
            if(document.getElementById('memberId')) document.getElementById('memberId').value = "";

            // 1. Xóa trắng các ô nhập liệu một cách an toàn
            ...
            // 2. Tăng số thứ tự lên 1 an toàn
            const orderEl = document.getElementById('childOrder');
            if(orderEl) {
                orderEl.value = parseInt(orderEl.value || 0) + 1;
            }
            
            // 3. Trỏ chuột lại vào ô Họ Tên để gõ luôn người tiếp theo
